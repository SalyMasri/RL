...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        if np.random.rand() < epsilon:
            # pick a random action from the allowed actions
            action = np.random.choice(all_actions)
        else:
        # pick the best action among allowed_actions, based on Q
        # i.e. argmax Q[state, a] but only over allowed_actions
            q_values_allowed = [Q[state, a] for a in all_actions]
            best_index_in_allowed = np.nanargmax(q_values_allowed)
            action = all_actions[best_index_in_allowed]        # ADD YOUR CODE SNIPPET BETWEEN EX 4.1
        ...
        # It is recommended you use the np.random module
        progress = min(float(current_total_steps) / float(anneal_timesteps), 1.0)
        epsilon = epsilon_initial + progress * (epsilon_final - epsilon_initial)
        ...
        if np.random.rand() < epsilon:
            ...
            q_values_allowed = [Q[state, a] for a in all_actions]
            best_index_in_allowed = np.nanargmax(q_values_allowed)
            action = all_actions[best_index_in_allowed]
        # ADD YOUR CODE SNIPPET BETWEENEX  4.2
        ...
        while episode < self.episode_max  and diff > self.threshold:
            ...
            while not end_episode:
                ...
                action = epsilon_greedy(
                    ...
                    eps_type="linear"  # or "constant" depending on what you want
                )
                # ADD YOUR CODE SNIPPET BETWEEN EX 5
                ...
                td_target = R + discount * np.nanmax(Q[s_next, :])
                td_error = td_target - old_value
                Q[s_current, action] = old_value + lr * td_error
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
    ...
    def random_agent(self):
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                n[s_current, action] += 1                # ADD YOUR CODE SNIPPET BETWEEN EX. 1.2
